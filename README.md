...
Telegram-бот на `aiogram`, который принимает ссылки на треки Яндекс Музыки и отправляет короткую карточку с двумя действиями:
...
- Работает в inline mode и в личке
- В inline-режиме отправляет обычное сообщение с эмодзи `🐈`
- В личке отправляет обычное сообщение бота с custom emoji, если Telegram разрешает его для владельца бота
...
```text
https://music.yandex.ru/album/2448178/track/21404459
```

В ответ бот отправит такое же сообщение уже от себя, с кнопками и custom emoji вместо обычного префикса, если Telegram разрешает его для этого бота.
